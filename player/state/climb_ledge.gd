...
func update(delta:float):
	...
	tween.interpolate_property(p,"global_position",p.global_position, Vector2(p.global_position.x, final_pos.y), 0.1,Tween.TRANS_LINEAR)
	tween.interpolate_property(owner.sprite,"position",owner.sprite.position, Vector2(0.0, 4.0),0.1,Tween.TRANS_LINEAR)
